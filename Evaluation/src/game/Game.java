...
public class Game {
	...
	private void initialisation() {
		...
		String w;
		do {
			System.out.println("Bonjour ! Une partie d'échec ? 1- Oui 	2-Non");
			w = entree.nextLine();
		} while (!w.equals("1"));
		...
		if (w.equals("1")) {
			...
			w = entree.nextLine();
			listeJoueurs.get(0).setNom(w);
			
			System.out.println("Entrez le nom du joueur 2");
			w = entree.nextLine();
			listeJoueurs.get(1).setNom(w);
			System.out.println(listeJoueurs.get(0).getNom() + " VS " + listeJoueurs.get(1).getNom());
			init_pion();
		}
	...
	private void init_pion() {
		int x = 1, y = 2;
		for (x = 1; x <= 8; x++) {
			new Pion(x, y, Couleur.WHITE);
		}
		y = 7;
		for (x = 1; x <= 8; x++) {
			new Pion(x, y, Couleur.BLACK);
		}
	...
	private Position getEntree(int n) {
		String temp = null;
		if (n == 1)
			temp = "Entrez la position de départ (x puis y)";
		if (n == 2)
	...
	private void update() {
		...
		for (Roi count : Roi.tabRoi) {
			...
			if (count.isEchec() && !count.isEchecEtMat())
				System.out.println("Le roi " + count.getCouleur() + " est en échec");
			else if (count.isEchecEtMat()) {//(count.isEchec() && count.isEchecEtMat() || count.isEchecEtMat()) {
				over = true;
